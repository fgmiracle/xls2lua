...
def splitbysymbol(source, symbol=","):
    result_list = []
    char_stack = []
    match_stack = []
    length = len(source)
    ...
    for i in range(length):
        ...
        elif char in revert_match_table and match_table[match_stack[-1]] == char:
            match_stack.pop()

        if (char == symbol or i==length-1) and len(match_stack) <= 0:
            unit_str = "".join(char_stack).strip()
            result_list.append(unit_str.strip(','))
            char_stack = []
            continue
...
if __name__ =='__main__':
    if len(sys.argv) < 3:
        print("please input file or output dir")
        sys.exit(1)
    
    convet_xls_file(sys.argv[1], sys.argv[2])
